...
public struct PackageImporter {
    ...
    public func importPackage(at package: URL, to project: URL, format: BuildInfoFormat) throws {
        guard !fileManager.itemExists(at: project) else {
            throw SwiftPkgError.projectExists("Directory \(project.path) already exists.")
        }
    ...
    private func importBundlePackage(_ package: URL, project: URL, format: BuildInfoFormat) throws {
        ...
        guard distributionFiles.isEmpty else {
            throw SwiftPkgError.importFailed("Bundle-style distribution packages are not supported for import. Consider importing the included sub-package(s).")
        }
    ...
    private func handleDistributionPackage(project: URL) throws {
        ...
        guard packages.count == 1 else {
            throw SwiftPkgError.importFailed("Distribution packages to be imported must contain exactly one component package! Found: \(packages)")
        }
    ...
    private func convertPackageInfo(package: URL, project: URL, format: BuildInfoFormat) throws {
        ...
        guard let parser = XMLParser(contentsOf: url) else { throw SwiftPkgError.importFailed("Could not parse \(url.path)") }
        let delegate = PackageInfoParser()
        parser.delegate = delegate
        guard parser.parse() else { throw SwiftPkgError.importFailed("Could not parse \(url.path): \(parser.parserError?.localizedDescription ?? "invalid XML")") }
    ...
    private func convertInfoPlist(package: URL, project: URL, format: BuildInfoFormat) throws {
        ...
        do {
            ...
            throw SwiftPkgError.importFailed("Could not read \(url.path): \(error.localizedDescription)")
        }
        guard let plist = object as? [String: Any] else { throw SwiftPkgError.importFailed("Could not read \(url.path)") }
