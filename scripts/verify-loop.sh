...
DEFAULT_BUILD_OUTPUT="$WORK/default-build.stdout"
printf '+ %s\n' "$BIN --export-bom-info $PROJECT"
"$BIN" --export-bom-info "$PROJECT" > "$DEFAULT_BUILD_OUTPUT"
grep -q '^pkgbuild: Wrote package to ' "$DEFAULT_BUILD_OUTPUT"
QUIET_BUILD_OUTPUT="$WORK/quiet-build.stdout"
printf '+ %s\n' "$BIN --quiet $PROJECT"
"$BIN" --quiet "$PROJECT" > "$QUIET_BUILD_OUTPUT"
test ! -s "$QUIET_BUILD_OUTPUT"
